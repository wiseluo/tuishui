...
function gc_upload_image(el) {
    ...
    //   return false;
    // }
    // console.log("中数"+el.files.length);
...
function upload_file(el) {
  ...
  if ($(el).data('file') == 'excel') {
    ...
    tip = "文件格式不支持，请选择png/gif/bmp/jpeg/jpg/pdf/doc/docx格式的文件,文件需小于2M！";
    for (var i = 0, f; i < el.files.length; i++) {
      f = el.files[i];
      if (!/\.jpg$|\.png$|\.gif$|\.bmp$|\.jpeg$|\.pdf$|\.doc$|\.docx/i.test(f.name)) {
        layer.msg(tip);
      // } else if (el.files[0].size > 1024 * 1024 * 2) {
      //   layer.msg('文件大小超限,请选择小于2M的文件!');
      } else {
...
//订单页面数据交互
function order_lzc(checkUrl, logistics, outerDiv, outerTitle, pid, btn_content,cdata) {
  $.ajax({
    type: "Get",
    url: checkUrl,
      ...
      layer.open({
        ...
        yes: function (i, stage) {
          ...
          if (outerTitle != '审核') {
            ...
            if (outerTitle == '修改' && outerDiv == 'orddetail') {
              $('[name=status]').val('3')
            }
          } else {
            ...
            postUrl='/member/order/examine/'+cdata.id
          }
          $('#start_port').change()
...
function getdetail(current) {
  ...
  if (read) {
    // 添加订单判断
    if (current.data().api == "member/order?outer=orddetail&tp=update" || current.data().api == 'member/order?outer=orddetail&tp=examine') {
      checkUrl = '/' + apiArr[0] + '/' + read + cdata.id
      postUrl = '/' + apiArr[0] + post
      outerTitle = outerTitle
      order_lzc(checkUrl, 0, outerDiv, outerTitle, pid, btn_content,cdata)
    }
    else {//正常流程
        ...
        success: function (str) {
          ...
          layer.open({
            ...
            success: function () {
              ...
              if (cus !== -1 && postUrl !== '/member/customer') {
                console.log(2222222)
                // for(var i=0;i<window.AllMData.length;i++){
            ...
            yes: function (i, stage) {
              ...
              if (flag && addPro(pid)) {
                ...
                if (ad == 4) {
                  ...
                  //报关登记点击确定->待退税
                  if (current.data().api == "filing?outer=fildetail&tp=call") {
                    var arr = data.split('&');
                    arr[0] = "status=1";
                    data = arr.join('&');
                  }
    ...
    switch (searchStr) {
      case 'checkFile':
        postUrl = '/member/' + apiArr[0].split('/')[1] + '/update/' + cdata.id;
        break;
      case 'allot':
        postUrl = '/' + apiArr[0] + '/' + cdata.id;
        break;
      // case 'conTainEr':
      //   postUrl = '/member/' + apiArr[0].split('/')[1] + '/container/' + cdata.id;
      //   break;
    }
...
function xiazai(data) {
  window.location.href = '/member/download/billing_data/' + data.id;
}

//下载报关资料
function generator(data) {
  window.location.href = '/member/download/' + data.id;
}
//结案
function over(data) {
...
//重置密码
function reset(data) {
    layer.confirm('确定要重置密码吗？', {btn: ['确定','取消']}, function (i) {
        var post = '/member/personalcenter/personal/password/reset';
        $.post(post, {id: data.id}, function (res) {
            // var result = JSON.parse(res);
            if(res.code == 200){
                getData($('[data-gctable]'));
...
$(document).on('click', '[data-select]', function () {
  ...
  if (regNumber.test(all_api)) {
    ...
    api = '/member/' + v + '/choose' + spstr;
  } else {
    api = '/member/' + $(this).data().select + '/choose'
  }
  var ele,
...
$(document).on('click', '.accdetail #company_name', function (obj) {
  ...
  $.get('/member/companies/choose', function (str) {
    var index = layer.open({
      type: 1,
      title: outerTitle,
      area: '780px',
...
$(document).on('click', '.cusdetail #customer_user', function (obj) {
  ...
  var myId = $(this).attr('id');
  $.get('/member/account/choose', function (str) {
    var index = layer.open({
      type: 1,
      title: outerTitle,
      area: '780px',
...
//采购员选择 业务编号选择
$(document).on('click', '#buyerId,#bnote_num', function (obj) {
  var ele,
    $this = $(this),
    url = '',
    outerTitle = $this.parent().prev().text();
  ...
  $.get('/member/bnotelist',{action:'html'}, function (str) {
    var index = layer.open({
      type: 1,
      title: outerTitle,
      area: '780px',
      ...
      yes: function (i) {
        if (myId == 'buyerId') {
          var name = $('.buyerTbody tr.bg-active').children('.u_truename').html();
          var a = $this.parent().parent().index();
          $('.buyer')[a].value = name;
          $('.u_name')[a].value = $('.buyerTbody tr.bg-active').children('.u_name').html();
        } else {
          ...
          $.get('/member/bnoteinfo/' + bnote_id, function (res) {
...
//     return false;
//   } else {
//     var api = '/member/' + $this.data().followed + '/choose/' + drawer_id;
//   }
//   var pendingRequests = {};
//   jQuery.ajaxPrefilter(function (options, originalOptions, jqXHR) {
//     var key = options.url;
...
//     return false;
//   } else {
//     var api = '/member/' + $this.data().follow + '/choose/' + drawer_id;
//   }
//   var pendingRequests = {};
//   jQuery.ajaxPrefilter(function (options, originalOptions, jqXHR) {
//     var key = options.url;
...
//           var order_id = ele.find('.bg-active').data('id'),
//             bpi = '/member/clearance/' + order_id;
//           adtt = '/member/data/father/' + 13;
//           $($this.data('target')).val(order_id);
//           $this.val(ele.find('.bg-active td').eq(0).html());
//           $.get(bpi, {}, function (v) {
...
$(document).on('click', '.btn[data-check]', function () {
  ...
  api = '/member/' + $(this).data().check + '/choose',
    outerWidth = $(this).data().outwidth,
    pclass = $(this).data().content,
    ptbody = $(pclass),
    outerTitle = $(this).parent().prev().text();
  if ($('.tsjs').index() != -1) {
    ...
    if (!customer_id) {
      ...
      var api = '/member/' + $this.data().check + '/choose/' + customer_id;
    }
  }
  var pendingRequests = {};
  jQuery.ajaxPrefilter(function (options, originalOptions, jqXHR) {
...
$(document).on('click', '[data-order]', function () {
  var api = '/member/' + $(this).data().order + '/read/' + $(this).data().id,
    outerWidth = $(this).data().outwidth,
...
//                     <h5 class="am-panel-title">产品名称<input readonly id="name" value="`+b.product.name+`"></h5>
//                   </header>
//                   <div class="pro_sec">
...
//                           <input id="" value="`+(b.product.brand!=null?b.product.brand.classify_str:'')+`" disabled="disabled">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label>品牌类型：</label>
//                           <input id="name" value='`+(b.product.brand!=null?b.product.brand.type_str:'')+`' disabled="disabled">
//                         </div>
//                       </li>
//                       <li class="last_li">
//                         <div>
//                           <label><small class="red">*</small>英文品名：</label>
//                           <input   id="en_name" data-parsley-required  data-parsley-trigger="change" value='`+b.product.en_name+`' disabled="disabled">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label><small class="red">*</small>境内货源地：</label>
//                           <input placeholder="请填写境内货源地" data-parsley-required  data-parsley-trigger="change" name="pro[` + indexTr + `][domestic_source]" id="domestic_source">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label>产地：</label>
//                           <input placeholder="请填写产地" name="pro[` + indexTr + `][production_place]" id="production_place">
//                         </div>
//                       </li>
//                       <li  class="last_li">
//                         <div>
//                           <label><small class="red">*</small>原产国(地区)：</label>
//                           <input placeholder="请填写原产国" data-parsley-required  data-parsley-trigger="change" name="pro[` + indexTr + `][origin_country]" id="origin_country">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label><small class="red">*</small>最终目的国(地区)： </label>
//                           <input placeholder="请填写最终目的国" data-parsley-required  data-parsley-trigger="change" value="" name="pro[` + indexTr + `][destination_country]" id="destination_country">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label><small class="red">*</small>货物属性：</label>
//                           <select class="removeClass" name="pro[` + indexTr + `][goods_attribute]" data-parsley-required  data-parsley-trigger="change" id="goods_attribute">
...
//                           <input placeholder="请填写HSCode" value="`+b.product.hscode+`" disabled="disabled">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label>CIQ编码：</label>
//                           <input name="pro[` + indexTr + `][ciq_code]" id="ciq_code">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label>退税率：</label>
//                             <input name="pro[` + indexTr + `][tax_refund_rate]" id="tax_refund_rate" data-parsley-required data-parsley-trigger="change" value="`+b.product.tax_refund_rate+`" readonly>
//                         </div>
//                       </li>
//                       <li class="last_li">
//                         <div>
//                           <label>种类：</label>
//                           <input placeholder="请填写种类" name="pro[` + indexTr + `][species]" id="species">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label>表面材质：</label>
//                           <input placeholder="请填写表面材质" name="pro[` + indexTr + `][surface_material]" id="surface_material">
//                         </div>
//                       </li>
//                       <li class="last_li" style="width:67%">
//                         <div>
//                           <label>款号：</label>
//                           <input placeholder="请填写款号" name="pro[` + indexTr + `][section_number]" id="section_number">
//                         </div>
//                       </li>
//                       <li class="last_li" style="width:100%">
//                         <div>
//                           <label>出口货物在最终目的国（地区）是否享受优惠关税：</label>
//                           <select class="removeClass" name="pro[` + indexTr + `][enjoy_offer]" id="enjoy_offer">
...
//                           <input placeholder="请填写最大包装件数" name="pro[` + indexTr + `][pack_number]" data-parsley-required  data-parsley-trigger="change" id="pack_number" class="pack_number">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label> <small class="red">*</small>净重(KG)：</label>
//                           <input placeholder="请填写净重(KG)" data-parsley-required  data-parsley-trigger="change" name="pro[` + indexTr + `][net_weight]" id="net_weight" class="net_weight">
//                         </div>
//                       </li>
//                       <li class="last_li">
//                         <div>
//                           <label> <small class="red">*</small>毛重(KG)：</label>
//                           <input placeholder="请填写总毛重(KG)" data-parsley-required  data-parsley-trigger="change"  name="pro[` + indexTr + `][total_weight]" id="total_weight" class="total_weight">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label> <small class="red">*</small>体积(CBM)：</label>
//                           <input placeholder="请填写体积(CBM)" data-parsley-required  data-parsley-trigger="change" name="pro[` + indexTr + `][volume]" id="volume" class="volume">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label> <small class="red">*</small>产品数量：</label>
//                           <input placeholder="请填写产品数量" data-parsley-required  data-parsley-trigger="change" name="pro[` + indexTr + `][number]" id="number" class="number">
//                         </div>
//                       </li>
//                       <li class="last_li">
//                         <div>
//                           <label>单价(USD)：</label>
//                           <input placeholder="请填写单价(USD)" name="pro[` + indexTr + `][single_price]" id="single_price" class="single_price" readonly>
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label>单位：</label>
//                           <input placeholder="请填写单位" name="pro[` + indexTr + `][unit]" id="unit" class="unit">
//
//                         </div>
//                       </li>
//
//                       <li>
//                         <div>
//                           <label> <small class="red">*</small>货值(USD)：</label>
//                           <input placeholder="请填写货值(USD)" data-parsley-required  data-parsley-trigger="change" name="pro[` + indexTr + `][total_price]" id="total_price" class="total_price">
//                         </div>
//                       </li>
//                       <li class="last_li">
//                         <div>
//                           <label><small class="red">*</small>开票金额(RMB)：</label>
//                           <input placeholder="开票金额(RMB)"  data-parsley-required  data-parsley-trigger="change" name="pro[` + indexTr + `][value]" id="value" class="value">
//                         </div>
//                       </li>
//                       <li>
//                         <div>
//                           <label><small class="red">*</small>计量单位：</label>
//                           <input disabled="disabled" value="`+b.product.measure_unit_data.name+`">
//                         </div>
//                       </li>
//                       <li class="last_li" style="width: 67%;">
//                         <div>
//                           <label>法定数量和单位：</label>
//                           <input placeholder="请填写数量" class="default_num" name="pro[` + indexTr + `][default_num]" id="default_num">
//                           <input id="default_unit" class="default_unit"  name="pro[` + indexTr + `][default_unit]" placeholder="请填写法定单位">
//                         </div>
//                       </li>
...
//         // '</td>' +
//         // '<td>' +
//         // '<input type="text" data-parsley-required autocomplete="off" data-parsley-trigger="change" value="' + b.product.measure_unit_data.name + '" class="measure_unit text-center" readonly>' +
//         // '</td>' +
//         // '<td>' +
...
//         // '</td>' +
//         // '<td>' +
//         // '<input type="text" data-parsley-required  data-parsley-trigger="change" name="pro[' + indexTr + '][default_unit]" class="default_unit text-center">' +
//         // '</td>' +
//         // '<td>' +
...
//         // '</td>' +
//         // '<td>' +
//         // '<input type="text" data-parsley-type="number" data-parsley-trigger="change" class="text-center exchange_cost" readonly>' +
//         // '<input type="hidden" name="pro[' + indexTr + '][rate]" value="' + b.product.rate + '">' +
//         // '</td>' +
...
//     var $th_p = (b.pivot.number - (typeof (v.product_count[b.id]) == 'undefined' ? 0 : v.product_count[b.id].amount))
...
//         '<td>' +
//         '<select class="form-control gog-select text-center huiv tax_refund_rate" name="inv[' + b.id + '][tax_refund_rate]" data-parsley-required data-parsley-trigger="change" ></select>' +
//         '</td>' +
