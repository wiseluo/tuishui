...
<div class="paydetail">
    <form class="form-horizontal rem-ittee" style="overflow: hidden;">
        ...
        <div class="col-sm-4 m-t">
            <div class="form-group">
                ...
                <div class="col-sm-5">
                    <input type="button" class="text-center form-control btn-s btn-primary addBill" value="添加收汇单"/>
                </div>
            </div>
        </div>
        <div class="panel-body table-responsive panel" style="overflow-x:auto;width:100%;text-align: center">
 ...
 <script>
    $(function(data){
        $('.addBill').off('click').on('click',function () {  //添加订单信息
            var ele,
                $this = $(this),
                outerWidth = '600px'
        ...
        function billList(page){  //订单列表
            let customer_id = $('.customer_id').val()
                api = '/member/finance/payreceipt/choose'
            $.ajax({
                type: 'get',
                url: api,
                data: {
                ...
                success: function(res){
                    ...
                    if(res.code == 200){
                        ...
                        $.each(res.data.data,function (i,v) {
                            html += `<tr class="bill_choose" data-id="${v.id}">
                                <td class="number">${v.identifier}</td>
                                <td class="money">${v.rmb}</td>
                            </tr>`
                        })
                        $('.billList').html(html)
                        //调用分页
                        Creatpage('page',total, billList)
                    }
                }
            })
        }
        $(document).off('dblclick','.bill_choose')
        $(document).on('dblclick','.bill_choose',function () {  //双击选择收汇单
            var billHtml = ''
                bill_index = $(".j_bill_list_item").length;
            var billNum = $(this).find('.number').text();
                money = $(this).find('.money').text();
                id = $(this).data('id')
                billHtml += `<tr class="j_bill_list_item">
                        <td>${billNum}</td>
                        <input name="receipt[${bill_index}][receipt_id]" type="hidden" class="id" value="${id}" />
                        <td>${money}</td>
                        <td><input name="receipt[${bill_index}][pay_money]" class="price" /></td>
                        <td></td>
                        <td></td>
                        <td class="receipt_del">删除</td>
                      </tr>`
            $('.bill_listTable tbody').append(billHtml)
            layer.close(layer.index);
        })
        //删除当前商品信息
        //删除当前关联信息
        $(document).on('click','.receipt_del',function () {  //删除订单
            $(this).closest('tr').remove();
            $(".j_bill_list_item").each(function(index,item){
