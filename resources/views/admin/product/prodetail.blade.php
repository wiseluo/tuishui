    ...
    <form class="form-horizontal cusName" style="overflow: hidden;">
        ...
        <div class="col-sm-4 m-b">
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <input type="text" id="cusName" class="form-control choose_Cus"
                    ...
                    <input type="hidden" id="customer_id" name="customer_id" value="{{ isset($product) ? $product->customer_id : ''}}" {{ $disabled }}>
                </div>
            </div>
        </div>
        <div class="col-sm-4 m-b">
        ...
        <div class="col-sm-4 m-b">
            <div class="form-group">
                <label for="proEnName" class="col-sm-4 control-label necessary_front">产品英文名称</label>
                <div class="col-sm-8">
                    <input type="text" id="proEnName" name="en_name" class="form-control" data-parsley-required data-parsley-trigger="change"
        ...
        @include('admin.common.dataselect', ['label'=>'退税率(%)', 'var'=>'tax_refund_rate', 'obj'=>isset($product) ? $product : '', 'disabled'=> $updateDoneDisabled])
        <div class="col-sm-4 m-b">
            <div class="form-group">
        ...
        <div class="col-sm-4 m-b">
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <input type="text" id="proUnit" name="unit" class="form-control" value="{{ isset($product) ? $product->unit : ''}}" {{ $updateDoneDisabled }}/>
                </div>
            </div>
        </div>
        <div class="col-sm-4 m-b">
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <input type="text" class="brand form-control" value="{{ isset($product->brand) ? $product->brand->name : ''}}" readonly {{ $updateDoneDisabled }}/>
        ...
        <div class="col-sm-4 m-b">
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <input type="text" class="link form-control" value="{{ isset($product->link) ? $product->link->name : ''}}" data-parsley-required data-parsley-trigger="change" readonly {{ $updateDoneDisabled }}/>
                    <input type="hidden" class="link_id" name="link_id" value="{{ isset($product->link) ? $product->link->id : ''}}" {{ $updateDoneDisabled }}/>
                </div>
            </div>
        </div>
        @include('admin.common.dataselect', ['label'=>'申报单位', 'var'=>'measure_unit', 'obj'=> isset($product) ? $product : ''])
        <div class="clearfix"></div>
        <div class="col-sm-12 table-responsive" style="margin: 10px 0">
        ...
        <div class="form-group" style="margin: 20px 0">
            <label for="proPic" class="col-sm-3 control-label pic" style="width:12%">图片</label>
        ...
        <div class="form-group">
            <label class="col-sm-3 control-label" style="width:12%"></label>
            <div class="col-sm-3 img1">
        ...
        <div class="col-sm-12 m-t">
            <div class="form-group">
                ...
                <div class="col-sm-11">
                    <textarea id="proNote" name="remark" class="form-control remarks" {{ $updateDoneDisabled }} rows="3">{{ isset($product) ? $product->remark : ''}}</textarea>
                </div>
            </div>
        </div>
        <div class="clearfix"></div>
        @include('admin.common.approve', ['obj'=>isset($product) ? $product : ''])
    </form>
</div>
<script>
    $(function () {
    ...
    $(document).off('click','.brand')
    $(document).on('click','.brand',function () {  //关联品牌
        $.ajax({
            ...
            success: function (str) {
                ...
                layer.open({
                    ...
                    yes: function (i,layero) {
                        let id = layero.find('.bg-active').data('id')
                            name = layero.find('.bg-active .name').text()
                        $('.brand').val(name)
                        $('.brand_id').val(id)
                        layer.close(i)
                    }
    ...
    function showStandard(number, name, arr, disabled) {
        arr = arr.split('|');
        $.getJSON('/admin/custom_sys/element', {
            hscode: number
        }).done(function(ret){
    ...
    function getGoodsByEnter(page,value) {
        ...
        $.getJSON('/admin/custom_sys/product_list', {
            keyword: value,
            page: page
        }).done(function (res) {
            console.log(res);
