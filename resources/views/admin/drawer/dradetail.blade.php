...
<div class="dradetail panel-body">
    <form class="form-horizontal" style="overflow: hidden;">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <input type="text" id="cusName" class="form-control choose_Cus" placeholder="点击选择客户" data-parsley-required data-parsley-trigger="change" value="{{ isset($drawer->customer) ? $drawer->customer->name : ''}}" {{ $updateDoneDisabled }} readonly/>
                <input type="hidden" id="customer_id" value="{{ isset($drawer) ? $drawer->customer_id : ''}}" name="customer_id" {{ $updateDoneDisabled }}>
            </div>
        </div>
        <div class="form-group">
            <label for="drawerComName" class="col-sm-3 control-label necessary_front">开票人公司名称</label>
            <div class="col-sm-8">
                <input type="text" id="company" name="company" class="form-control" data-parsley-required data-parsley-trigger="change"  value="{{ isset($drawer) ? $drawer->company : ''}}" {{ $updateDoneDisabled }}/>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <input type="text" id="telephone" name="telephone" class="form-control" data-parsley-required data-parsley-trigger="change"  value="{{ isset($drawer) ? $drawer->telephone : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group">
            <label for="taxIdtNumber" class="col-sm-3 control-label necessary_front">纳税人识别号</label>
            <div class="col-sm-8">
                <input type="text" id="tax_id" name="tax_id" class="form-control" placeholder="营业执照上的号" data-parsley-required data-parsley-trigger="change"  value="{{ isset($drawer) ? $drawer->tax_id : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <!-- <div class="form-group">
            <label for="bLRN" class="col-sm-3 control-label necessary_front">营业执照注册号</label>
            <div class="col-sm-8">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <input type="text" id="tax_at" onclick="laydate()"  name="tax_at" class="form-control" placeholder="此项需与提交的“一般纳税人认定书”照片信息保持一致" data-parsley-required data-parsley-trigger="change"  value="{{ isset($drawer) ? $drawer->tax_at : ''}}" {{ $updateDoneDisabled }} readonly/>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <input type="text" id="address" name="address" class="form-control" placeholder="即工厂地址，需和“营业执照”地址一致，如不一致，需提供证明材料。" data-parsley-required data-parsley-trigger="change"  value="{{ isset($drawer) ? $drawer->address : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <select id="export" class="form-control" data-parsley-required data-parsley-trigger="change" id="export" name="export" {{ $updateDoneDisabled }}>
                    <option value="0" {{ (isset($drawer) && $drawer->export==0) ? 'selected="selected"' : '' }}>有</option>
                    <option value="1" {{ (isset($drawer) && $drawer->export==1) ? 'selected="selected"' : '' }}>无</option>
                </select>
            </div>
        </div>
        <div class="form-group export_register">
            <label for="customs_code" class="col-sm-3 control-label">海关注册登记编码</label>
            <div class="col-sm-8">
                <input type="text" id="customs_code" name="customs_code" class="form-control" value="{{ isset($drawer) ? $drawer->customs_code : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group export_record hide">
            <label for="tax_customs_code" class="col-sm-3 control-label">税局备案海关代码</label>
            <div class="col-sm-8">
                <input type="text" id="tax_customs_code" name="tax_customs_code" class="form-control" value="{{ isset($drawer) ? $drawer->tax_customs_code : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label necessary_front">经营范围</label>
            <div class="col-sm-8">
                <input type="text" id="busines_scope" name="busines_scope" class="form-control" value="{{ isset($drawer) ? $drawer->busines_scope : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label">法人代表</label>
            <div class="col-sm-8">
                <input type="text" id="corporate_repre" name="corporate_repre" class="form-control" value="{{ isset($drawer) ? $drawer->corporate_repre : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label">开户行</label>
            <div class="col-sm-8">
                <input type="text" id="bankname" name="bankname" class="form-control" value="{{ isset($drawer) ? $drawer->bankname : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label">银行账号</label>
            <div class="col-sm-8">
                <input type="text" id="bankaccount" name="bankaccount" class="form-control" value="{{ isset($drawer) ? $drawer->bankaccount : ''}}" {{ $updateDoneDisabled }}/>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-2">
                ...
                <input type="hidden" id="pic_verification" name="pic_verification" value="{{ isset($drawer) ? $drawer->pic_verification : ''}}" data-parsley-required data-parsley-trigger="change">
            </div>
            <div class="col-sm-2">
                <a href="#" class="btn btn-s-md btn-primary necessary_front" data-upload="image" data-input="[name=pic_register]">营业执照副本</a>
                <input type="hidden" id="pic_register" name="pic_register" value="{{ isset($drawer) ? $drawer->pic_register : ''}}" data-parsley-required data-parsley-trigger="change">
            </div>
             <div class="col-sm-2">
                <a href="#" class="btn btn-s-md btn-primary necessary_front" data-upload="image" data-input="[name=pic_brand]">厂牌</a>
                <input type="hidden" id="pic_brand" name="pic_brand" value="{{ isset($drawer) ? $drawer->pic_brand : ''}}" data-parsley-required data-parsley-trigger="change">
           </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-2">
                ...
                <input type="hidden" id="pic_production" name="pic_production" value="{{ isset($drawer) ? $drawer->pic_production : ''}}" data-parsley-required data-parsley-trigger="change">
           </div>

             <div class="col-sm-2">
                <a href="#" class="btn btn-s-md btn-primary" data-upload="image" data-input="[name=pic_business_license]">税务登记证副本照片</a>
                <input type="hidden" id="pic_business_license" name="pic_business_license" value="{{ isset($drawer) ? $drawer->pic_business_license : ''}}">
           </div>
              <div class="col-sm-4">
                <a href="#" class="btn btn-s-md btn-primary" data-upload="image" data-input="[name=pic_home]">厂房租赁合同(或房产证)</a>
                <input type="hidden" id="pic_home" name="pic_home" value="{{ isset($drawer) ? $drawer->pic_home : ''}}">
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label"></label>
            <div class="col-sm-2 ">
                ...
                <input type="hidden" id="pic_receipts_income" name="pic_receipts_income" value="{{ isset($drawer) ? $drawer->pic_receipts_income : ''}}">
            </div>
            <div class="col-sm-2 ">
                <a href="#" class="btn btn-s-md btn-primary" data-upload="image" data-input="[name=pic_sales_invoice]">销项发票</a>
                <input type="hidden" id="pic_sales_invoice" name="pic_sales_invoice" value="{{ isset($drawer) ? $drawer->pic_sales_invoice : ''}}">
            </div>
            <div class="col-sm-2 ">
                <a href="#" class="btn btn-s-md btn-primary" data-upload="image" data-input="[name=pic_credit_rating]">纳税信用等级</a>
                <input type="hidden" id="pic_credit_rating" name="pic_credit_rating" value="{{ isset($drawer) ? $drawer->pic_credit_rating : ''}}">
            </div>
            <div class="col-sm-3 ">
                <a href="#" class="btn btn-s-md btn-primary" data-upload="image" data-input="[name=pic_customs_certificate]">海关注册登记证书</a>
                <input type="hidden" id="pic_customs_certificate" name="pic_customs_certificate" value="{{ isset($drawer) ? $drawer->pic_customs_certificate : ''}}">
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label"></label>
            <div class="col-sm-2 ">
                ...
                <input type="hidden" id="tax_return" name="tax_return" value="{{ isset($drawer) ? $drawer->tax_return : ''}}">
            </div>
            <div class="col-sm-3 ">
                <a href="#" class="btn btn-s-md btn-primary" data-upload="image" data-input="[name=pic_warehouse_photos]">仓库照片</a>
                <input type="hidden" id="pic_warehouse_photos" name="pic_warehouse_photos" value="{{ isset($drawer) ? $drawer->pic_warehouse_photos : ''}}">
            </div>
            <div class="col-sm-4 ">
                <a href="#" class="btn btn-s-md btn-primary" data-upload="image" data-input="[name=pic_other]">其他</a>
                <input type="hidden" id="pic_other" name="pic_other" value="{{ isset($drawer) ? $drawer->pic_other : ''}}">
            </div>
        </div>
        ...
        @include('admin.common.approve', ['obj'=> isset($drawer) ? $drawer : ''])
    </form>
</div>
<script>
    $(function(){
        ...
        $(document).on('click','.addPro',function () {
            var outerTitle = '选择产品'
                api = '/admin/product/product_choose';
            $.ajax({
                url: api,
                type: 'get',
                data: {
                ...
                success: function (str) {
                    ...
                    layer.open({
                        ...
                        yes: function (i,layero) {
                            let obj = []
                                newTrs = layero.find('.bnoteTbody input:checked').closest('tr');
                            if(newTrs.length){
