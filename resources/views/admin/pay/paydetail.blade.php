...
<div class="paydetail m-t">
    <form class="form-horizontal rem-ittee" style="overflow: hidden;">
        ...
        @include('admin.common.dataselect', ['label'=>'款项类型', 'var'=>'type', 'obj'=> isset($pay) ? $pay : ''])
        <div class="col-sm-4 m-b">
            <div class="form-group">
        ...
        <div class="col-sm-4 m-b">
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <input type="text" id="companyName" value="{{ isset($pay->company->name) ? $pay->company->name : '' }}" class="form-control choose choose_company" data-parsley-required data-parsley-trigger="change" readonly {{ $disabled }}/>
                    <input type="hidden" class="company_id" name="company_id" value="{{ isset($pay) ? $pay->company_id : '' }}" {{ $disabled }}>
                </div>
            </div>
        </div>
        <div class="col-sm-4 m-b">
        ...
        @endif
        <div class="clearfix"></div>
        @include('admin.common.approve', ['obj'=>isset($pay) ? $pay : ''])
    </form>
</div>
<script>
    $(function(data){
        ...
        $('.selectOpen').off('click').on('click',function (){
            ...
            if($(this).attr('id') == "cusName"){
                ...
                api = '/admin/finance/remittee_choose';
            }
            $.get(api, {action:'html'}, function (str) {
                ...
                var index = layer.open({
                    ...
                    content: str,
                    yes: function (i) {
                        if( api == '/admin/finance/remittee_choose'){
        ...
        function orderList(page){
            let type = $('#type').val()
                ...
                api = '/admin/finance/payorder/choose'
            $.ajax({
                type: 'get',
                url: api,
                data: {
        ...
        laydate(endDate);
        // $('.paydetail').on('keyup','[name="money"]',function(){
        //     var money = parseFloat($(this).val());
        //     var balance = 0,tax_balance = 0
        //     balance = $('.balance').val();
        //     tax_balance = $('.tax_balance').val();
        //     if(money > (balance*1+tax_balance*1)){
        //         layer.msg("金额不能大于结汇退税余额之和，请重新输入");
        //         $(this).val("");
        //     }
        // })
    })
