...
class LoginController extends Controller
{
    ...
    */
    ...
    public function locallogin($result, $request)
    {
        if($result->code == 200){
            ...
            if(count($user_res) == 0){
                $params = array(
                    ...
                    'usercenter_token'=> $result->token,
                    'type'=> 1
                );
                $id = DB::table('users')->insertGetId($params);
                // DB::table('notifications')->insert(['sender'=> 0, 'receiver'=> 1, 'content'=> '请完善账号：'. $request->input('username') .' 的信息', 'status'=> 1, 'created_at'=> date('Y-m-d H:i:s')]);
            }else{
                DB::table('users')->where('id', $user_res->id)->update(['center_uid'=> $center_uid, 'usercenter_token'=> $result->token]);
            }
        }else{
        ...
        if ($this->guard()->attempt(['username' => $request->input('username'), 'password' => '123456', 'type'=> 1])) {
    ...
    public function redirectPath()
    {
        ...
        return property_exists($this, 'redirectTo') ? $this->redirectTo : '/member/home';
    }
}
