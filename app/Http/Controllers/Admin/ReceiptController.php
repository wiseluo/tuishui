...
use App\Http\Requests\Admin\ReceiptPostRequest;
...
class ReceiptController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.receipt.index', $this->receiptService->renderStatus());
    }

    public function read(DataRepository $dataRepository, $type = 'read', $id = 0)
    {
        $currency_id = $dataRepository->getTypesByFatherId(5);
        $view = view('admin.receipt.recdetail', compact('currency_id'));
        if ($id) {
    ...
    {
        $receipt = $this->receiptService->find($id);
        return view('admin.receipt.recdetail_number', compact('receipt'));
    }
