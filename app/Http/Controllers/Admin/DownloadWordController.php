...
class DownloadWordController extends Controller
{
    ...
    public function billingData(ClearanceRepository $repository, $id)
    {
        ...
        $filename = $save_path . $clearance->order->ordnumber.'.zip';
    ...
    public function createDoc(ClearanceRepository $repository, $id)
    {
        ...
        foreach($proArr as $k => $v){
            ...
            $textrun1->addText($clearance->order->ordnumber, array('underline'=>'single'));
            $textrun2 = $section->createTextRun();
            $textrun2->addText('甲方（供方）：');
            $maxLength = 90;
            $textrun2->addText($v[0]->drawer->company, array('underline'=>'single'));
            $textrun2->addText(str_repeat(' ', $maxLength - strlen($v[0]->drawer->company)));
            $textrun2->addText('签约日期:');
            $textrun2->addText(date('Y-m-d', strtotime($clearance->order->customs_at ." -45 day")), array('underline'=>'single'));
            $textrun3 = $section->createTextRun();
            $textrun3->addText('乙方（需方）：');
            $maxLength = 90;
            $textrun3->addText($company->invoice_name, array('underline'=>'single'));
            $textrun3->addText(str_repeat(' ', $maxLength - strlen($company->invoice_name)));
            $textrun3->addText('签约地点：');
            $textrun3->addText('义乌', array('underline'=>'single'));

            $section->addText('根据中华人民共和国合同法的有关规定,经双方协商签订本合同,以资共同信守。');
            ...
            $table->addCell($cellWidth, $cellVCentered)->addText('金额（含税）', null, $cellHCentered);
            ...
            foreach($v as $i => $n){
                ...
                $table->addCell($cellWidth, $cellVCentered)->addText($n->pivot->measure_unit_cn, null, $cellHCentered);
            ...
            $section->addText('一、付款条件：发货后三个月内付款并开具真实、有效、合法的增值税专用发票。');
            $section->addText('二、质量要求：根据国家质量标准及行业质量标准,与需方确认样一致。');
            $section->addText('三、包装要求：符合进口国外贸标准包装。');
            $section->addText('四、运输条款：');
            //$textrun4 = $section->createTextRun();
            // $textrun4 = $section->addTextRun();
            // $textrun4->addText('checkbox: ');
            // $textrun4->addFormField('checkbox')->setDefault(true);
            // $section->addFormField('checkbox')->setDefault(true)->setValue('Your name');
            // $textrun5 = $section->createTextRun();
            // $textrun5->addText('checkbox2:');
            // $textrun5->addFormField('checkbox')->setDefault(true);

            $section->addText('  A.送到苏福路185号，运费由供方承担；B.送到需方指定地点，运费由供方承担；');
            $section->addText('  C.工厂装柜，运费由 供方  需方承担；');
            $section->addText('五、交货时间：'. $clearance->order->customs_at .'之前');
            $section->addText('六、违约责任：');
            $section->addText('  1、甲、乙双方严格履行合同规定的各项条款，如一方违约，由违约方依约承担违约责任。');
            $section->addText('  2、产品质量不符合约定的，乙方可要求更换或退货，若由于甲方所供应产品的质量问题给乙方或者第三方造成损害的，乙方有权要求甲方承担一切责任，甲方应当赔偿乙方因此所受损失（包括但不限于运费、人员交通费、索赔所支付的律师费、诉讼费等）。');
            $section->addText('  3、若甲方迟延交货，每迟延一日，应当向乙方支付合同总金额的0.5%作为违约金。');
            $section->addText('七、其他：');
            $section->addText('八、争议解决：');
            $section->addText('  1、本合同未尽事宜，由双方另行签订补充协议，补充协议与本合同具有同等法律效力。');
            $section->addText('  2、本合同一式两份，甲、乙双方各执一份，具有同等法律效力，自甲乙双方盖章之日起生效。');
            $section->addText('  3、本合同产生的任何争议，甲、乙双方应本着积极的态度进行友好协商解决；协商不成的由合同签订地或者乙方所在地人民法院提起诉讼。');
            
            $phpWord->addTableStyle('myTable2', $tableStyle);
            $table2 = $section->addTable('myTable2');
            $table2->addRow();
            $table2->addCell($cellWidth, $cellVCentered)->addText('甲方名称：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($v[0]->drawer->company, null, $cellHLeft);
            $table2->addCell($cellWidth, $cellVCentered)->addText('乙方名称：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($company->invoice_name, null, $cellHLeft);
            $table2->addRow();
            $table2->addCell($cellWidth, $cellVCentered)->addText('法人代表：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($v[0]->drawer->corporate_repre, null, $cellHLeft);
            $table2->addCell($cellWidth, $cellVCentered)->addText('法人代表：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($company->corporate_repre, null, $cellHLeft);
            $table2->addRow();
            $table2->addCell($cellWidth, $cellVCentered)->addText('开户行及帐号：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($v[0]->drawer->bankname.$v[0]->drawer->bankaccount, null, $cellHLeft);
            $table2->addCell($cellWidth, $cellVCentered)->addText('开户行及帐号：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($company->bankname.$company->bankaccount, null, $cellHLeft);
            $table2->addRow();
            $table2->addCell($cellWidth, $cellVCentered)->addText('传真电话：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($v[0]->drawer->telephone, null, $cellHLeft);
            $table2->addCell($cellWidth, $cellVCentered)->addText('传真电话：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($company->telephone, null, $cellHLeft);
            $table2->addRow();
            $table2->addCell($cellWidth, $cellVCentered)->addText('地址：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($v[0]->drawer->address, null, $cellHLeft);
            $table2->addCell($cellWidth, $cellVCentered)->addText('地址：', null, $cellHCentered);
            $table2->addCell($cellWidth*2, array('gridSpan' => 2, 'vMerge' => 'restart', 'valign'=>'center'))->addText($company->address, null, $cellHLeft);
