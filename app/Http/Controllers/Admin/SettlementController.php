...
use App\Http\Requests\Admin\SettlementPostRequest;
...
class SettlementController extends Controller
{
    ...
    public function index(Request $request, $status = 0)
    {
        ...
        return view('admin.settlement.index', $renderStatus);
    }
    
    public function update(Request $request, $id)
    {
        $res = $this->settlementService->updateService($request->input(), $id);
        if($res['status']){
    ...
    {
        $res = $this->settlementService->approveService($request->input(), $id);
        if($res['status']){
            return response()->json(['code'=> 200, 'msg'=> $res['msg']]);
        }else{
            return response()->json(['code'=> 403, 'msg'=> $res['msg']]);
    ...
    {
        $view = view('admin.settlement.seldetail');
    ...
    {
        $view = view('admin.settlement.settlementdetail');
        $settlement = $this->settlementService->find($id);
        ...
        $drawers = array_map(function($item)use($received_invoice, $paid_deposit, $paid_payment, $refund_tax){
            if($item['bill_base'] == 1){ // 计费基数，1含税开票金额、2报关金额、3退税金额、4不含税开票金额
            ...
            }else if($item['bill_base'] == 4){
                $commission = bcmul(bcdiv($item['invoice_amount'], bcadd(1, bcdiv($item['tax_rate'], 100, 2), 2), 2), bcdiv($item['service_rate'], 100, 2), 2); //不含税开票金额=含税开票金额/(1+税率)
            }else{
                $commission = 0;
            }
    ...
    public function settlesave(SettlementPostRequest $request, $id)
    {
        ...
        if($settlement_res){
            return response()->json(['code'=>200, 'msg'=> '结算成功']);
        }else{
            return response()->json(['code'=>200, 'msg'=> '结算失败']);
        }
    }
