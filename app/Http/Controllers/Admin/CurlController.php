...
class CurlController extends Controller
{
    ...
    //去erpapp获取客户详情
    public function erpCustomer(Curl $curl, $id) {
        $url = config('app.erpapp_url'). "/customeralls/". $id . '?token=' . session('center_token');
        $response = $curl->get($url);
        return $response->response;
    }

    //去erpapp获取客户列表
    public function erpCustomerList(Curl $curl, Request $request) {
        $post_data = array(
        ...
        $erpCustomers = $curl->get(config('app.erpapp_url'). '/customeralls', $post_data)->response;
        return $erpCustomers;
    }
