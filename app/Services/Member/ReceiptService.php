...
class ReceiptService
{
    ...
    protected $zjportService;

    public function __construct(ReceiptRepository $receiptRepository, OrderRepository $orderRepository, ZjportService $zjportService)
    {
    ...
    public function binding($param, $id)
    {
        ...
        $scmOrderextStatus = $this->zjportService->ScmOrderext($param, $order_id);

        if($orderStatus){
            DB::commit();
            return ['status'=>'1', 'msg'=> '关联成功-品浙行发送'.$scmOrderextStatus['msg']];
        }else{
            DB::rollback();
            return ['status'=>'0', 'msg'=> '修改失败-品浙行发送'.$scmOrderextStatus['msg']];
        }
    }
