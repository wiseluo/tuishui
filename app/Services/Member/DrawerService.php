...
class DrawerService
{
    public function __construct(DrawerRepository $drawerRepository, NotificationRepository $notificationRepository)
    {
    ...
    {
        return $this->drawerRepository->renderStatus();
    }
    ...
    {
        $res = $this->drawerRepository->save($param);
        if($res){
    ...
    public function approve($param, $id)
    {
        ...
        if($res){
            ...
            $data = [];
            $data['cid'] = $param['cid'];
            $data['receiver_id'] = $drawer->created_user_id;
            $data['content'] = '您提交的【开票人管理】'. $drawer->company;
            if($param['status'] == 3){
                $data['content'] .= '已审核通过';
            }else{
                $data['content'] .= '被审核拒绝';
