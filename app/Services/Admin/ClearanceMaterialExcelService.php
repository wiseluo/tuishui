...
class ClearanceMaterialExcelService
{
    ...
    public function customs($clearance, $excel, $number){
        ...
        $cell = [
        	...
        	'G7' => $order->company->exemption_code,
    ...
    public function invoiceExcel($clearance, $excel, $number)
    {
        ...
        if(isset($order->trader)){
            $trader_info = "TO:". $order->trader->name;
        ...
        $cell = [
            ...
            //'F14' => isset($order->priceClauseData->name) ? $order->priceClauseData->name .':' : '',
            //'G14' => isset($order->shipmentport->port_e_cod) ? $order->shipmentport->port_e_cod : '',
        ];
    ...
    //         'B41' => 'N/M',
    //         'B46' => $this->rowchr([$trader_info]),
    //         'N40' => $order->ordnumber,
    //         'N41' => date('Y/m/d', strtotime($order->shipping_at)),
    //         'N42' => $unloadingport,
    ...
    public function customsContractExcel($clearance, $excel, $number)
    {
        ...
        if(isset($order->trader)){
            $trader_info = $order->trader->name;
            //$trader_info = $order->trader->name .", ". $order->trader->address;
        }else{
            $trader_info = "";
        }
        $cell = [
        ...
        $index = $index +9;
        $cell['A'. $index] = isset($order->receive->name) ? $order->receive->name .', '. $order->receive->address : '';
