...
class DrawerRepository
{
    ...
    public function drawerIndex($param)
    {
        ...
        return Drawer::where(function($query)use($keyword){
            ...
            ->where(function($query)use($pid){
            	$query->when($pid, function($query)use($pid){
            		return $query->whereHas('product', function ($query) use ($pid){
            			$query->where('products.id', '=', $pid);
            		});
            	});
            })
            ->where($where)
    ...
    public function getDrawerList($param)
    {
        $where = [];
        $userlimit = [];
        if(isset($param['userlimit'])){
    ...
    public function drawerList($param)
    {
        $keyword = isset($param['keyword']) ? $param['keyword'] : '';
        $pageSize = isset($param['pageSize']) ? $param['pageSize'] : 10;
        return Drawer::where(function($query)use($keyword){
    ...
    {
        $product_where = $param['userlimit'];
        $product_where[] = ['customer_id', '=', $param['customer_id']];

        $keyword = isset($param['keyword']) ? $param['keyword'] : '';
        $pageSize = isset($param['pageSize']) ? $param['pageSize'] : 10;
        return DrawerProduct::with(['drawer', 'product'])
        ->whereHas('product', function($query)use($product_where){
            $query->where('status', 3)->where($product_where);
        })
    ...
    //搜索开票人名称获取开票人及客户
    public function searchDrawerByNameRepository($param)
    {
        $where = [];
        $where[] = ['status', '=', 3];
        $where[] = ['cid', '=', 0];
        $where[] = ['company', '=', $param['keyword']];
        return Drawer::where($where)
            ->with(['customer'])
            ->get()
            ->toArray();
    }

    //根据客户id搜索开票人名称获取开票人信息
    public function searchDrawerByNameCustomerIdRepository($param)
    {
        $where = [];
        $where[] = ['status', '=', 3];
        $where[] = ['cid', '=', 0];
        ...
        return Drawer::where($where)->get()->toArray();
    }
